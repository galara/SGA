...
public class Categoriaproducto extends javax.swing.JInternalFrame {
    ...
    Statement sent;
    String[] titulos = {"Id", "Nombre Categoria", "Fech_Reg", "Estado"};
    ...
    public void setFiltroTexto() {
        Helper.setFiltraEntrada(nombre.getDocument(), FiltraEntrada.NUM_LETRAS, 45, true);
        Helper.setFiltraEntrada(txtDato.getDocument(), FiltraEntrada.NUM_LETRAS, 45, true);
    }
    ...
    private void formatotabla() {
        ...
        for (int i = 0; i < 4; i++) {
            column = categoria.getColumnModel().getColumn(i);
            if (i == 1) {
                column.setPreferredWidth(200); //Difine el ancho de la columna
                categoria.getColumnModel().getColumn(i).setCellRenderer(modelocentrar);
            } else if (i == 0) {
                column.setPreferredWidth(5); //Difine el ancho de la columna
                categoria.getColumnModel().getColumn(i).setCellRenderer(modelocentrar);//Para centrar los datos en las columnas
            } else if (i == 2) {
    ...
    private void Llenar() {
        try {
            ...
            ResultSet rs = sent.executeQuery(/*sql*/sqlcat.LLENAR + sqlcat.ORDER_BY);// especifica la consulta y la ejecuta
    ...
    private void MostrarTodo(String Dato) {
        try {
            ...
            conn = BdConexion.getConexion();
            String sql = "";
            sql = sqlcat.BUSCANOMBRE + Dato + sqlcat.CUALQUIERA;
            removejtable();
            //model = new DefaultTableModel(null, titulos);//objeto TableModel para proporcionar los datos del objeto ResultSet al objeto JTable
            sent = conn.createStatement();// crea objeto Statement para consultar la base de datos
    ...
    private void filaseleccionada() {
        ...
        if (categoria.getValueAt(fila, 0) != null) {
            try {
                //conn = BdConexion.getConexion();
                conn = BdConexion.getConexion();
                Habilitar();
                String sql = sqlcat.LLENAR + sqlcat.WHERE + sqlcat.ID + sqlcat.IGUAL + categoria.getValueAt(fila, 0);
    ...
    private void initComponents() {
        ...
        setName("Categoriaproducto"); // NOI18N
        ...
        categoria.setModel(model = new DefaultTableModel(null, titulos)
            {
                @Override
            ...
            categoria.setDoubleBuffered(true);
            categoria.setDragEnabled(true);
            categoria.setFocusCycleRoot(true);
            categoria.setRowHeight(24);
            categoria.setSurrendersFocusOnKeystroke(true);
            ...
            jLabel8.setText("<--Categoria de Producto-->");
            ...
            buttonMostrar.setMnemonic(KeyEvent.VK_F10);
            buttonMostrar.setText("Mostrar Todo");
            buttonMostrar.setToolTipText("ALT+F10");
        ...
        if (nombre.getText().equals("") || getFecha() == null) {
            ...
            if (resp == 0) {
                try {
                    ...
                    PreparedStatement ps = conn.prepareCall(sqlcat.NUEVOC);
                    ...
                    if (e.getErrorCode() == 1062) {
                        JOptionPane.showInternalMessageDialog(this,
                                "Error al guardar \n La Categoria ya existe ingrese una diferente", "Error ", JOptionPane.ERROR_MESSAGE);
                    } else {
                        JOptionPane.showInternalMessageDialog(this,
    ...
    private void bntEliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bntEliminarActionPerformed
        ...
        if (resp == 0) {
            try {
                ...
                conn = BdConexion.getConexion();
                int fila = categoria.getSelectedRow();
                String sql = sqlcat.DELETEC + sqlcat.WHERE + sqlcat.ID + sqlcat.IGUAL + categoria.getValueAt(fila, 0);
                ...
                if (e.getErrorCode() == 1451) {
                    JOptionPane.showInternalMessageDialog(this,
                            "Error al eliminar \n La categoria esta siendo utilizada por un material y no puede ser eliminada ", "Error ", JOptionPane.ERROR_MESSAGE);
                } else {
                    JOptionPane.showInternalMessageDialog(this,
                            "Error al eliminar \n Verifique los datos e intente nuevamente", "Error ", JOptionPane.ERROR_MESSAGE);
                }
        ...
        if (nombre.getText().equals("") || getFecha() == null) {
            ...
            if (resp == 0) {
                try {
                    ...
                    conn = BdConexion.getConexion();
                    int fila = categoria.getSelectedRow();
                    String dao = (String) categoria.getValueAt(fila, 0);

                    //PreparedStatement nos permite crear instrucciones SQL compiladas, que se ejecutan con más efi ciencia que los objetos Statement
                    //también pueden especifi car parámetros,lo cual las hace más fl exibles que las instrucciones Statement
                    PreparedStatement ps = conn.prepareCall(sqlcat.UPDATEC);
                    ps.setString(1, nombre.getText());
                    ps.setString(2, getFecha());
