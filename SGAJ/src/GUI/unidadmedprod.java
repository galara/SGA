 ...
 * @author GLARA
 */
public class unidadmedprod extends javax.swing.JInternalFrame {

    DefaultTableModel model;
    Connection conn;//getConnection intentara establecer una conexión.
    Statement sent;
    String[] titulos = {"Id", "Unidad de Medida", "Fech_Reg", "Estado"};
    SimpleDateFormat formatof = new SimpleDateFormat("dd/MM/yyyy");

    /**
     * Creates new form Unidad de medida
     */
    public unidadmedprod() {
        initComponents();
        Desabilitar();
        Llenar();
        setFiltroTexto();
    ...
    private void formatotabla() {
        ...
        for (int i = 0; i < 4; i++) {
            column = unidades.getColumnModel().getColumn(i);
            if (i == 1) {
                column.setPreferredWidth(200); //Difine el ancho de la columna
                unidades.getColumnModel().getColumn(i).setCellRenderer(modelocentrar);
            } else if (i == 0) {
                column.setPreferredWidth(5); //Difine el ancho de la columna
                unidades.getColumnModel().getColumn(i).setCellRenderer(modelocentrar);//Para centrar los datos en las columnas
            } else if (i == 2) {
    ...
    private void Llenar() {
        try {
            ...
            ResultSet rs = sent.executeQuery(/*sql*/sqlun.LLENAR + sqlun.ORDER_BY);// especifica la consulta y la ejecuta

            String[] fila = new String[4];
            while (rs.next()) {

                fila[0] = rs.getString("idunidad");
    ...
    private void initComponents() {
        ...
        unidades.setModel(model = new DefaultTableModel(null, titulos)
            {
                @Override
            ...
            unidades.setCursor(new java.awt.Cursor(java.awt.Cursor.DEFAULT_CURSOR));
            unidades.setDoubleBuffered(true);
            unidades.setDragEnabled(true);
            unidades.setFocusCycleRoot(true);
            unidades.setRowHeight(24);
            unidades.setSurrendersFocusOnKeystroke(true);
                ...
                try {
                    ...
                    if (e.getErrorCode() == 1062) {
                        JOptionPane.showInternalMessageDialog(this,
                                "Error al guardar \n La Unidad ya existe ingrese una diferente", "Error ", JOptionPane.ERROR_MESSAGE);
                    } else {
                        JOptionPane.showInternalMessageDialog(this,
        ...
        if (resp == 0) {
            try {
                ...
                if (e.getErrorCode() == 1451) {
                    JOptionPane.showInternalMessageDialog(this,
                            "Error al eliminar \n La Unidad esta siendo utilizada por un material y no puede ser eliminada ", "Error ", JOptionPane.ERROR_MESSAGE);
                } else {
                    JOptionPane.showInternalMessageDialog(this,
                            "Error al eliminar \n Verifique los datos e intente nuevamente", "Error ", JOptionPane.ERROR_MESSAGE);
                }
        ...
        if (nombre.getText().equals("") || getFecha() == null) {
            ...
            if (resp == 0) {
                try {
                    ...
                    //PreparedStatement nos permite crear instrucciones SQL compiladas, que se ejecutan con más efi ciencia que los objetos Statement
                    //también pueden especifi car parámetros,lo cual las hace más fl exibles que las instrucciones Statement
                    PreparedStatement ps = conn.prepareCall(sqlun.UPDATEC);
                    ps.setString(1, nombre.getText());
                    ps.setString(2, getFecha());
