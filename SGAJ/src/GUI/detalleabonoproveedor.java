 ...
 * @author 30234
 */
public class detalleabonoproveedor extends javax.swing.JInternalFrame {

    /**
     * Creates new form busquedaproducto
     */
    DefaultTableModel model;
    String[] titulos = {"Id", "Fecha", "Monto", "No.Compra", "No.Doc", "Usuario","Observación"};
    int estado = 0;
    ...
    private void formatotabla() {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            if (i == 0) {
                ...
                column.setPreferredWidth(26);//Difine el ancho de la columna
                detalleabonos.getColumnModel().getColumn(i).setCellRenderer(modelocentrar);
            }
            else if ( i== 6 ) {
                column.setPreferredWidth(255);//Difine el ancho de la columna
                detalleabonos.getColumnModel().getColumn(i).setCellRenderer(modelocentrar);
            }
        }
    }
    ...
    public void MostrarTodo(String Dato) {
        try {
            ...
            if (rs.next()) {//verifica si esta vacio, pero desplaza el puntero al siguiente elemento
                ...
                while (rs.next()) {
                    fila[0] = rs.getString("xpagardetalle.idxpagardetalle");
                    fila[1] = formatof.format(rs.getDate("xpagardetalle.fecha"));
                    fila[2] = rs.getString("xpagardetalle.monto");
                    fila[3] = rs.getString("compra.idcompra");
                    fila[4] = rs.getString("compra.numdoc");
                    fila[5] = rs.getString("usuario.nombreusuario");
                    fila[6] = rs.getString("xpagardetalle.observacion");
                    model.addRow(fila);
                    //count = count + 1;
                }
    ...
    private void initComponents() {
        ...
        setTitle("Detalle de Abonos a Proveedores");
        ...
        detalleabonos.setModel(model = new DefaultTableModel(null, titulos)
            ...
            jLabel21.setText("<-Detalle de Abonos Proveedor->");
